...
def total_score_grid(grid, avoid, next_to,fill):
    score = 0
    rows = len(grid)
    cols = len(grid[0])
...
def get_candidaties(actual, same, is_same, num, item, next_to, avoid, fill):
    rows = len(actual)
    cols = len(actual[0])
    size = item[0]["size"]
...
def search_best(actual_grid, same_grid, remaining_plants, next_to, avoid,fill, best_result, total_count, k=3, prev_name=None):
    placed_count = total_count - len(remaining_plants)

    if not remaining_plants:
        score = total_score_grid(actual_grid,avoid,next_to,fill)
    ...
    cands = get_candidaties(actual_grid, same_grid, is_same, num, item, next_to, avoid, fill)
    if not cands:
        score = total_score_grid(actual_grid,avoid,next_to,fill)
    ...
    for cand in cands:
        ...
        search_best(new_actual, new_same, remaining_plants[1:], next_to, avoid,fill, best_result, total_count, k, current_name)

def placing_with_backtracking(grid_plants, lists, next_to, avoid,fill, k=3):
    ...
    search_best(actual_grid, same_grid, expanded, next_to, avoid,fill, best_result, len(expanded), k)
...
def main():
    ...
    #avoid_space = int(input(("want to fill the garden even if less efficeint?")))
    #if avoid_space == "T" or next_Spacing == "t":
    #    fill = True
    #else:
    #    fill = False
    ...
    grid_plants, same_grid, cant_place = placing_with_backtracking(grid_plants, lists, next_to, avoid,fill, k=3)    
    print(cant_place)
    print()
