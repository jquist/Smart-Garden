...
def default_plant_description(plant):
    category = CATEGORY_LABELS.get(plant.plant_category, "garden plant")
    roles = _role_text(plant.plant_roles)
...
def default_planting_tips(plant):
    tips = []
    ...
    start = _clean(plant.plant_start)
    end = _clean(plant.plant_end)
    if start and end:
        tips.append(f"Typical outdoor planting window: {start} to {end}.")

    harvest_start = _clean(plant.harest_start)
    harvest_end = _clean(plant.harest_end)
...
def default_planting_how_to(plant):
    if plant.plant_category == "weed":
        return (
