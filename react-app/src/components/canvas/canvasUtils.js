...
function getPlantColourFamily(hue) {
...
export function getPlantTypeColour(name) {
  ...
  const borderLightness = 34 + ((hash >>> 4) % 11);
  const backgroundLightness = Math.min(borderLightness + 28, 74);
  const dotLightness = Math.max(borderLightness - 9, 24);

  return {
    label: `${getPlantColourFamily(hue)} ${hue}`,
    background: `hsla(${hue}, ${saturation}%, ${backgroundLightness}%, 0.34)`,
    border: `hsl(${hue}, ${saturation}%, ${borderLightness}%)`,
    dot: `hsl(${hue}, ${Math.min(saturation + 8, 78)}%, ${dotLightness}%)`,
  };
}
